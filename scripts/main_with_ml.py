# main_with_ml.py
# Enhanced version of main.py with optional ML prediction workflow.
# This extends your original toolkit with machine learning capabilities.
...
# ML imports
from ml_workflows import run_ml_workflow
...
# ============================================================
# Enhanced Tkinter GUI with ML option
# ============================================================

def interactive_setup_gui() -> dict:
    """
    Build Tkinter configuration window with ML options.
    Returns a config dict used by main().
    """
    cfg: dict = {}

    root = tk.Tk()
    root.title("Marine Heatwave Detector – Configuration (ML Enhanced)")
    ...
    # ML options
    enable_ml_var = tk.BooleanVar(value=True)
    forecast_window_var = tk.StringVar(value="7")
    ml_train_end_var = tk.StringVar(value="2019-12-31")
    run_cv_var = tk.BooleanVar(value=True)
    ...
    def on_submit():
        ...
        try:
            ...
            # ML config
            enable_ml = enable_ml_var.get()
            forecast_window = _parse_int(forecast_window_var.get(), "Forecast window") if enable_ml else None
            ml_train_end = _parse_date(ml_train_end_var.get(), "ML train end") if enable_ml else None
            run_cv = run_cv_var.get() if enable_ml else False
    ...
    # ML Options separator
    row += 1
    ttk.Separator(root, orient='horizontal').grid(row=row, column=0, columnspan=4, sticky='ew', pady=10)

    row += 1
    tk.Label(root, text="ML PREDICTION OPTIONS", font=('TkDefaultFont', 10, 'bold')).grid(
        row=row, column=0, columnspan=4, **pad
    )

    row += 1
    tk.Checkbutton(root, text="Enable ML prediction", variable=enable_ml_var).grid(
        row=row, column=0, columnspan=2, sticky="w", **pad
    )

    row += 1
    tk.Label(root, text="Forecast window (days):").grid(row=row, column=0, sticky="e", **pad)
    tk.Entry(root, textvariable=forecast_window_var, width=8).grid(row=row, column=1, **pad)
    tk.Label(root, text="ML train end date:").grid(row=row, column=2, sticky="e", **pad)
    tk.Entry(root, textvariable=ml_train_end_var, width=12).grid(row=row, column=3, **pad)

    row += 1
    tk.Checkbutton(root, text="Run cross-validation", variable=run_cv_var).grid(
        row=row, column=0, columnspan=2, sticky="w", **pad
    )

    row += 1
    tk.Button(root, text="Run Analysis", command=on_submit, bg='lightblue', font=('TkDefaultFont', 10, 'bold')).grid(
        row=row, column=0, columnspan=4, pady=15
    )
...
# ============================================================
# Main pipeline with ML integration
# ============================================================
...
def main():
    ...
    # Adjust total steps based on ML option
    total_steps = 9 if cfg["enable_ml"] else 8
    ...
    with tqdm(total=total_steps, desc="Overall progress", unit="step") as pbar:
        ...
        # 9) ML prediction (if enabled)
        if cfg["enable_ml"]:
            t0 = time.perf_counter()
            print("\n" + "=" * 70)
            print("RUNNING ML PREDICTION WORKFLOW")
            print("=" * 70)
            ...
            run_ml_workflow(
                ...
                train_end_date=cfg["ml_train_end"],
                run_cv=cfg["run_cv"],
            )
            timings["ML prediction"] = time.perf_counter() - t0
            pbar.update(1)
